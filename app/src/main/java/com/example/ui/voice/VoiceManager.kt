...
class VoiceManager(
    ...
    private val BACKGROUND_RESUME_DELAY_MS = 450L

    private val backgroundResumeRunnable = Runnable {
        if (!shuttingDown && allowBackgroundResume) resumeBackgroundVoice()
    }
    ...
    fun startListening() {
        mainHandler.post {
            ...
            mainHandler.removeCallbacks(backgroundResumeRunnable)
    ...
    private fun finishListening(errorMessage: String?, resume: Boolean) {
        ...
        onPartialSpeech("")
        if (resume && wasListening && allowBackgroundResume) {
            mainHandler.removeCallbacks(backgroundResumeRunnable)
            mainHandler.postDelayed(backgroundResumeRunnable, BACKGROUND_RESUME_DELAY_MS)
        }
